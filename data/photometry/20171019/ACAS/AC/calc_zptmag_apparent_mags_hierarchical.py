...
def main():
    ref = 'Ms'    # which photometry package should be used to compute zeropoints
    method = {'Md':'daophot', 'Ms':'sourceextractor'}
    photmethod = method[ref]
    ...
    pattern = 'C2[0-2]/*_F[0-9][0-9][0-9]W*.all'    # pattern for the photometry tables

    # cut out some fields we do not need to make indexing the data frame a little easier
    dref = 'd'+ref
    all_methods = list(method.keys())
    drop_methods = all_methods.copy()
    drop_methods.remove(ref)
    drop_fields = ['Xs', 'Ys', 'Xd', 'Yd', 'RAs', 'DECs', 'IMAGE'] + drop_methods + ['d'+x for x in drop_methods]
    keep_fields = [ref, 'd'+ref, 'EXPTIME', 'EPOCH']
    ...
    for i, fn in enumerate(files):
        fixfn = fn.replace('cy20','').replace('cy22', '').replace('_.all', '.all')
        obj, pb = os.path.basename(fixfn).replace('.all','').split('_')
        try:
            mags = at.Table.read(fn, format='ascii.commented_header')
        except ValueError as e:
            print(fn, "format inconsistent")
            continue
#        try:
#            mags.rename_column('texp', 'EXPTIME')
#        except KeyError as e:
#            pass
#
#        try:
#            mags.rename_column('EXPOCH', 'EPOCH')
#        except KeyError as e:
#            pass
#
#        try:
#            mags.rename_column('Epoch', 'EPOCH')
#        except KeyError as e:
#            pass

        mags.keep_columns(keep_fields)

        # mask out junk values
        mask = (mags[dref] < 0.5) & (np.abs(mags[ref]) < 50) & (mags['EXPTIME'] >= mintime)
        if len(mags[mask]) == 0:
            print(fn, "No data", ref, obj, pb)
            continue
        nbad = len(mags[~mask])
        if nbad > 0:
            print(fn, nbad, "bad obs", ref, obj, pb)
        mags = mags[mask]

        # save a few extra columns to help identify objects in the combined tables
        nmags = len(mags)
        data  = [[obj,]*nmags, [pb,]*nmags]
        names = ['objID','pb']
        for index, name in enumerate(names):
            mags[name] = data[index]

        # combine the photometry
        all_mags = mag_table.get(pb, None)
        new_mags = mags.to_pandas()

        # save everything into a combined dataframe per passband
        if all_mags is None:
            all_mags = new_mags
        else:
            all_mags = pd.concat([all_mags, new_mags])
        mag_table[pb] = all_mags
    ...
    for i, pb in enumerate(mag_table):
        ...
        cycle_flag = [ 1 if x <= 14.1 else 0 for x in all_mags['EPOCH'] ]
        ...
        with pm.Model() as hierarchical:
            # parameters from intrinsic dispersion and the zeropoint in each passband
            ...
            # the likelihood of the intrinsic dispersion and the zeropoint given the standards apparent mags and instrumental mags
            ...
            fig2.savefig('htrace_{}_{}.pdf'.format(photmethod, pb))
    ...
    for c in cols:
        if out2[c].dtype == np.float64:
            out2[c].format = '%.6f'
    print(out2)
    out2.write('WDphot_{}.dat'.format(photmethod), format='ascii.fixed_width', delimiter='  ', overwrite=True, fill_values=[(ascii.masked, 'NaN')])

    objID = out['objID']
    nobj = len(objID[nvar:])

    fig_big = plt.figure(figsize=(12,12))
    with PdfPages('{}.pdf'.format(photmethod)) as pdf:
        ...
        fig_big.savefig('{}_combined.pdf'.format(photmethod))
        plt.close(fig_big)
